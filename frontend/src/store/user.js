...
export const useUserStore = defineStore('user', () => {
  ...
  const userRole = computed(() => base.user.value?.role || '')
  const hasItsm = computed(() => base.permissions.value?.itsm || false)
  const hasOps = computed(() => base.permissions.value?.ops || false)
  const hasAdmin = computed(() => base.permissions.value?.admin || false)
  const isAdmin = computed(() => ['admin', 'super_admin'].includes(base.user.value?.role))
  const isSuperAdmin = computed(() => base.user.value?.role === 'super_admin')
