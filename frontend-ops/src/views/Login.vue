  ...
  <div class="login-page">
    <div class="login-card">
      ...
      <el-form :model="form" @submit.prevent="handleLogin">
        <el-tabs v-model="loginMode">
          <el-tab-pane label="飞书ID登录" name="feishu">
            <el-input v-model="form.feishu_user_id" placeholder="飞书用户ID" size="large" />
          </el-tab-pane>
          <el-tab-pane label="用户名登录" name="name">
            <el-input v-model="form.name" placeholder="用户名" size="large" />
          </el-tab-pane>
        </el-tabs>
      ...
      <div class="demo-hints">
        ...
        <el-tag size="small" @click="quickLogin('admin')">管理员</el-tag>
        <el-tag size="small" type="warning" @click="quickLogin('agent_1')">张三</el-tag>
      </div>
    </div>
  </div>
</template>
